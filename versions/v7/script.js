...
function getDifficulty(lvl) {
    ...
    const shapes = ['none', 'heart', 'diamond', 'plus', 'ring'];
    let shapeType = 'none';
    if (lvl >= 15 && Math.random() > 0.5) {
...
function generateRandomLevel(cols, rows, numSnakes, numRocks, shapeType) {
    ...
    for(let i=0; i<numSnakes; i++) {
        movingObstacles.forEach(mo => mo.track.forEach(pt => grid[pt.x][pt.y] = false));
            ...
            for(let y=0; y<rows; y++) {
                ...
                for(let d=0; d<4; d++) {
                    ...
                    while(cx >= 0 && cx < cols && cy >= 0 && cy < rows) {
                        if (grid[cx][cy]) { clear = false; break; }
                        cx += dx; cy += dy;
                    }
                    if (clear) validHeads.push({x, y, d});
                }
            }
        }
        ...
        movingObstacles.forEach(mo => mo.track.forEach(pt => grid[pt.x][pt.y] = true));
...
function isSnakeBlocked(snake) {
    ...
    while(true) {
        ...
        if (currentLevel.movingObstacles) {
            for (const mo of currentLevel.movingObstacles) {
                if (mo.track[mo.pos].x === cx && mo.track[mo.pos].y === cy) {
                    return true;
                }
            }
        }
...
function animationLoop(timestamp) {
    if (timestamp - lastMOTime > 400) {
        lastMOTime = timestamp;
        if (currentLevel && currentLevel.movingObstacles) {
            currentLevel.movingObstacles.forEach(mo => {
                if (mo.track.length > 1) {
                    let isSnakeCrossing = false;
                    for (const id of animatingSnakes) {
                        const snake = snakes.find(s => s.id === id);
                        if (snake && snake.p.some(pt => pt.x === mo.track[mo.pos].x && pt.y === mo.track[mo.pos].y)) {
                            isSnakeCrossing = true;
                        }
                    }
                    if (!isSnakeCrossing) {
                        mo.pos += mo.dir;
                        if (mo.pos >= mo.track.length - 1) {
                            mo.pos = mo.track.length - 1;
                            mo.dir = -1;
                        } else if (mo.pos <= 0) {
                            mo.pos = 0;
                            mo.dir = 1;
    ...
    if (timestamp > nextIdleAnimTime) {
        nextIdleAnimTime = timestamp + 3000 + Math.random() * 7000; // 3s to 10s
        const idleSnakes = snakes.filter(s => !animatingSnakes.has(s.id) && !isSnakeBlocked(s));
        if (idleSnakes.length > 0) {
            ...
            if (headG) {
                ...
                headG.classList.add('flick');
                setTimeout(() => {
                    headG.classList.remove('idle-look');
                    headG.classList.remove('flick');
                }, 2000);
            }
        }
    }
